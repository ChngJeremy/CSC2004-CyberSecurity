...
def create_PIL_image(data, resolution):
    ...
    image = Image.new("RGBA", resolution) # Create a new Image object
    ...
    for y in range(height):
        for x in range(width):
            ...
            try:
                ...
                r_hide, g_hide, b_hide, a_hide = secret_image[x,y]
                ...
                a_hide = get_n_most_significant_bits(a_hide, n_bits)

                # Remove least 'n' significant bits of image to hide in,
                # to store the 'n' most significant bits in that position
                r_hide_in, g_hide_in, b_hide_in, a_hide_in= carrier_image[x,y]
                ...
                a_hide_in = remove_n_least_significant_bits(a_hide_in, n_bits)
...
def decode(image_to_decode, n_bits):
    ...
    for y in range(height):
        for x in range(width):
            ...
            a_encoded = get_n_least_significant_bits(a_encoded, n_bits)
            ...
            a_encoded = shit_n_bits_to_8(a_encoded, n_bits)

            data.append((r_encoded, g_encoded, b_encoded, a_encoded))
